...
// Use the 'vi' global for mocking with Vitest
import { getRepositoryToken } from '@beenest/prisma';
import { vi } from 'vitest';

describe('CategoryController', () => {
  let controller: CategoryController;
  // Mock the Prisma CategoryDelegate using vi.fn()
  ...
  describe('deleteOneById', () => {
    it('should delete a category by its ID', async () => {
      const id = 1;
      const expectedResult = { id: 1, name: 'Deleted Category' };
  ...
  describe('updateOneByID', () => {
    it('should update a category by its ID', async () => {
      const id = 1;
      const select = { select: { id: true, name: true } };
      const expectedResult = { id: 1, name: 'Updated Category' };
